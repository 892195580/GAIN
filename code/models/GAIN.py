...
class GAIN_GloVe(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = BiLSTM(encoder_input_size, config)  # 10,

        self.gcn_dim = config.gcn_dim  # 64
        ...
        self.bank_size = self.config.gcn_dim * (self.config.gcn_layers + 1)  # 192 = 64 * (2+1)
        self.dropout = nn.Dropout(self.config.dropout)

        self.predict = nn.Sequential(
            nn.Linear(self.bank_size * 5 + self.gcn_dim * 4, self.bank_size * 2),  # 192*5+64*4, 192*2
            self.activation,
            self.dropout,
            nn.Linear(self.bank_size * 2, config.relation_nums),  # 192*2, 97
        )
    ...
    def forward(self, **params):
        ...
        for i in range(len(graphs)):
            ...
            x = torch.cat((output_h_t[i].unsqueeze(0), x), dim=0)  # 每篇doc先是一个doc节点，再拼接其他实体节点
        ...
        output_feature = torch.cat(output_features, dim=-1)  # 所有mention的每一层图卷积的结果拼接起来(num_nodes, 64*3)
        ...
        for i in range(len(graphs)):
            ...
            entity_bank[i, :select_metrix.size(0) - 1] = entity_representation[1:]  # 去掉doc节点的实体表达
            ...
            if entity_graph_feature is None:
                ...
                entity_graph_feature = torch.cat(  # 拼起来给下一层用，之前是mention特征，取出实体特征
...
class RelGraphConvLayer(nn.Module):
    ...
    def forward(self, g, inputs):
        ...
        g : DGLHeteroGraph
            Input graph.
        inputs : dict[str, torch.Tensor]  {'node': features}
        ...
        if self.use_weight:
            ...
            wdict = {}
        hs = self.conv(g, inputs, mod_kwargs=wdict)  # (num_nodes, 64)
...
class RelEdgeLayer(nn.Module):
    ...
    def forward(self, g, inputs):
        ...
        g.apply_edges(lambda edges: {  # 边的特征就是连接两节点的特征
