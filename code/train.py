...
def train(opt):
    ...
    if pretrain_model != '':
        ...
        logging("./logs/", 'load model from {}'.format(pretrain_model))
        start_epoch = chkpt['epoch'] + 1
        lr = chkpt['lr']
        logging("./logs/", 'resume from epoch {} with lr {}'.format(start_epoch, lr))
    else:
        logging("./logs/", 'training from scratch with lr {}'.format(lr))
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        logging("./logs/", 'start {}  eopch'.format(epoch))
        ...
        if epoch % opt.test_epoch == 0:
            ...
            model.train()
            logging("./logs/", '| epoch {:3d} | time: {:5.2f}s'.format(epoch, time.time() - eval_start_time))
            logging("./logs/", '-' * 89)
